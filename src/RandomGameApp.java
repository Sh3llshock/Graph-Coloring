...
public class RandomGameApp  {
    ...
    private static Pane overlayPaneLabel;
    private static Pane  overlayPaneNextNode;
    private static DrawVerticesAndEdgesFX drawObject;
    ...
    public void drawRandomGameApp(Stage primaryStage) {
        primaryStage.close();
        ...
        scene.getStylesheets().add(RandomGameApp.class.getResource("styles.css").toExternalForm());
        ...
        gc.fillText("Once you color a node and move to the next, ", 50, 350);
        gc.fillText("its color cannot be changed.", 50, 400);
        // gc.fillText("rightclicking", 50, 450);
        gc.fillText("Good luck!", 50, 450);
    ...
    public void loadRandomGenGraph() {
        ...
        LabelWithNode.clearLabelData ();//Tu
    ...
    private static VBox createSidePanel() {
    ...
    private Button createShowEdgeLabel() {
        ...
        labelButton.setOnAction(event -> {
            ...
            boolean isButtonVisible = overlayPaneLabel.getChildren().stream()
            .anyMatch(child -> child instanceof Button && child.isVisible());
    

            overlayPaneLabel.getChildren().forEach(child -> {
                if (child instanceof Button) {
                    ((Button) child).setVisible(!isButtonVisible);  
                }
                else if (child instanceof TextField) {
                    ((TextField) child).setVisible(false);  
                }
        });
        });
        ...
        switch (event.getButton()) {
            ...
            case SECONDARY:
                ...
                if (closestNodeIndex == RandomOrder.randomOrder[RandomOrder.currentNodeID]) {
                    // ColorsPicking.openColourPicker(gc, (int) event.getX() - 200, (int) event.getY());
                ...
                int[] currentColors = new int[localVertices.length];
                for (int i = 0; i < localVertices.length; i++) {
                    currentColors[i] = localVertices[i].colour;
                }
   ...
   //Tu: My apology, if I have  time, i will refactor this mess 
    ...
    private void createLabelonEdge () {
        ...
        for (HashMap.Entry<String, ArrayList<Double>> entry : temp.entrySet()) {
            ...
            overlayPaneLabel.getChildren().addAll(Label, labelButton);
    ...
    private static boolean areAdjacentNodesColoredDifferently(DrawnNodes[] localNodes) {
    ...
    private static void initUncoloured() {
        DrawnNodes[] localVertices = SharedData.getInstance().getDrawnCircles();
